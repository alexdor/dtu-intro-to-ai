...
class Mastermind:
    ...
    def get_white_and_black(self, combination, tested_code):
        ...
        tmp = combination[:]
        for i in range(4):
            if np.isin(tested_code[i], tmp):
                correct_colors += 1
                tmp = np.delete(tmp, np.argwhere(tmp == tested_code[i])[0])
    ...
    def do_minimax(self, array):
        ...
        for j in np.where(array[5] == 1)[0]:
            ...
            for i in np.where(
                array[4] == 1
            )[0]:  # index i corresponds to possible solution data set
...
def run(goal):
    ...
    # initialize the game
    game = Mastermind(np.array(goal, dtype=int))
...
def main():
    with open("test_perf.txt", "a") as file:
        file.write("[")
